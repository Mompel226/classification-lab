...
The same photographs the Life on Earth Hub shows under each group's name, copied from its repo so this
lab carries its own record. All from Wikimedia Commons, centre-cropped to 3 : 2, shown at 900 and 1400 px
wide, JPEG and WebP. Every one is public domain, CC0 or CC BY. The credit is also under every picture on
the page and in the "How to use" panel, from `js/tree.js` (the shared register in `labs-shared/tree/`).
...
## The silhouettes on the tree

From [PhyloPic](https://www.phylopic.org), CC0 or Public Domain Mark, copied in by `tools/build.mjs` from
